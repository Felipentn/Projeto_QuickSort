...
def quicksort(vet, inicio, fim):
    if inicio < fim:  # Verifica se ainda há elementos para ordenar
        pivo = particao(vet, inicio, fim)  # Obtém a posição correta do pivô
        quicksort(vet, inicio, pivo - 1)  # Ordena a primeira metade do vetor (antes do pivô)
        quicksort(vet, pivo + 1, fim)  # Ordena a segunda metade do vetor (após o pivô)
...
if __name__ == '__main__':
    vet = ler_arquivo_txt('path_1000')  # Lê o arquivo '1000NumerosAleatorios.txt'
    ...
    print("Tempo de execução: {} segundos".format(delta))  # Imprime o tempo de execução
